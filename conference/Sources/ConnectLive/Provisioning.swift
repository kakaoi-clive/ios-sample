...
class Provisioning: ObservableObject {
    ...
    @Published var serviceKey: String = "ICLEXMPLPUBL0KEY"
    @Published var secret: String = "YOUR0SRVC0SECRET"
    ...
    func signIn() async -> RoomError? {
        let result = await ConnectLive.signIn(serviceId: serviceId, key: serviceKey, secret: secret, endpoint: provisioningEndpoint)
        if result.0 == 0 {
            return nil
        } else {
            // 인증오류
